...
def get_test_response(intent):
    ...
    try:
        ...
        status_code = 404
    if slot1:
        speech_output = "The drone is "+ present_status +" commanding the drone to move towards "+ slot1 +" for "+ slot2 +" meters"
        flag = False
    else:
        flag = True
        speech_output = "the given is an unintended direction"
    dynamodb = boto3.resource('dynamodb')
    table = dynamodb.Table('drone_alexa')
    ...
    for i in connection_ids:
        apig_management_client = boto3.client('apigatewaymanagementapi', endpoint_url=f'https://g787lprx1k.execute-api.us-west-2.amazonaws.com/production')
...
def get_status_response(intent):
    ...
    try:
        ...
        logger.exception("couldn't find status")
        status_code = 404
    speech_output = "Present status of drone is " + present_status
    should_end_session = False
    return build_response(session_attributes, build_speechlet_response(
...
def get_takeoff():
    ...
    try:
        ...
        logger.exception("couldn't find status")
        status_code = 404
    speech_output = "The drone is " + present_status + " and commanding the drone to take off"
    should_end_session = False
    return build_response(session_attributes, build_speechlet_response(
        card_title, speech_output, None, should_end_session))
...
def get_rtl():
    ...
    try:
        ...
        logger.exception("couldn't find status")
        status_code = 404
    speech_output = "The drone is " + present_status + " and commanding the drone to return to base"
    should_end_session = False
    return build_response(session_attributes, build_speechlet_response(
        card_title, speech_output, None, should_end_session))
...
def get_welcome_response():
    ...
    session_attributes = {}
    card_title = "Welcome"
    speech_output = "Lets fly the drone. Getting all requirements ready!"
